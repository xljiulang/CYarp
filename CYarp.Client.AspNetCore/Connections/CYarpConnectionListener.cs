    ...
    sealed partial class CYarpConnectionListener : IConnectionListener
    {
        ...
        private async ValueTask<ConnectionContext?> AcceptCoreAsync(CancellationToken cancellationToken)
        {
            while (true)
            {
                ...
                {
                    Log.LogConnectError(this.logger, this.endPoint.ServerUri, "Operation canceled by user");
                    return null;
                }
        ...
        {
            [LoggerMessage(LogLevel.Information, "Connected to server {serverUri} successfully")]
            public static partial void LogConnected(ILogger logger, Uri serverUri);

            [LoggerMessage(LogLevel.Warning, "Disconnected from server {serverUri}")]
            public static partial void LogDisconnected(ILogger logger, Uri serverUri);

            [LoggerMessage(LogLevel.Warning, "Connect to server {serverUri} error: {reason}")]
