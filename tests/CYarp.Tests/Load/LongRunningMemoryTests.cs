...
public class LongRunningMemoryTests : RealConnectionTestBase
{
    public LongRunningMemoryTests(ITestOutputHelper output) : base(output)
    {
    }
    ...
    public async Task SSE_LongRunning_10Minutes_MemoryLeak()
    {
        ...
        Output.WriteLine($"[START] Initial memory: {startMemory / 1024 / 1024}MB");
        ...
        try
        {
            var stream = await GetSseStreamAsync(client, "site1");
            ...
            while (!cts.Token.IsCancellationRequested)
            {
                ...
                if (DateTime.UtcNow - lastSnapshot > snapshotInterval)
                {
                    ...
                    if (memorySnapshots.Count % 10 == 0) // Log every minute
                    {
                        ...
                        Output.WriteLine($"[{DateTime.UtcNow:HH:mm:ss}] Memory: {currentMemory / 1024 / 1024}MB (+{growthMB}MB), Events: {eventCount}");
                    }
                }
            }
        }
        ...
        Output.WriteLine($"[END] Final memory: {endMemory / 1024 / 1024}MB, Growth: {totalGrowthMB}MB, Events: {eventCount}");
        ...
        Output.WriteLine($"Average growth per snapshot: {avgGrowthPerSnapshot / 1024}KB");
    ...
    public async Task HTTP_LongRunning_ContinuousRequests_1Hour()
    {
        ...
        {
            var response = await SendRequestAsync(client, "site1", "/api/weather");
            ...
            if (DateTime.UtcNow - lastSnapshot > snapshotInterval)
            {
                ...
                Output.WriteLine($"[{elapsed:mm\\:ss}] Memory: {currentMemory / 1024 / 1024}MB (+{growthMB}MB), Requests: {requestCount}");
            }
        ...
        var endMemory = GC.GetTotalMemory(true);
        var totalGrowthMB = (endMemory - startMemory) / 1024 / 1024;
        Output.WriteLine($"[END] Final memory: {endMemory / 1024 / 1024}MB, Growth: {totalGrowthMB}MB, Requests: {requestCount}");
    ...
    public async Task SignalR_LongRunning_ConnectionChurn_30Minutes()
    {
        ...
        while (DateTime.UtcNow - startTime < duration)
        {
            ...
            if (DateTime.UtcNow - lastSnapshot > snapshotInterval)
            {
                ...
                var growthMB = (currentMemory - startMemory) / 1024 / 1024;
                var elapsed = DateTime.UtcNow - startTime;
                Output.WriteLine($"[{elapsed:mm\\:ss}] Memory: {currentMemory / 1024 / 1024}MB (+{growthMB}MB), Cycles: {connectionCycles}");
            }
        ...
        var endMemory = GC.GetTotalMemory(true);
        var totalGrowthMB = (endMemory - startMemory) / 1024 / 1024;
        Output.WriteLine($"[END] Final memory: {endMemory / 1024 / 1024}MB, Growth: {totalGrowthMB}MB, Cycles: {connectionCycles}");
    ...
    public async Task Mixed_LongRunning_AllProtocols_2Hours()
    {
        ...
        var tasks = new[]
        {
            ...
            Task.Run(async () =>
            {
                while (!cts.Token.IsCancellationRequested)
                {
                    await SendRequestAsync(client, "site1", "/api/weather");
            ...
            Task.Run(async () =>
            {
                while (!cts.Token.IsCancellationRequested)
                {
                    ...
                    var growthMB = (currentMemory - startMemory) / 1024 / 1024;
                    var elapsed = DateTime.UtcNow - startTime;
                    Output.WriteLine($"[{elapsed:hh\\:mm}] Memory: {currentMemory / 1024 / 1024}MB (+{growthMB}MB), HTTP: {stats.Http}, SSE: {stats.Sse}, SignalR: {stats.SignalR}");
                }
            })
        };
        ...
        var endMemory = GC.GetTotalMemory(true);
        var totalGrowthMB = (endMemory - startMemory) / 1024 / 1024;
        Output.WriteLine($"[END] Final memory: {endMemory / 1024 / 1024}MB, Growth: {totalGrowthMB}MB");
        Output.WriteLine($"Operations: HTTP: {stats.Http}, SSE: {stats.Sse}, SignalR: {stats.SignalR}");
    ...
    public async Task SSE_Multiple_LongRunning_Concurrent_30Minutes()
    {
        ...
        var monitorTask = Task.Run(async () =>
        {
            while (!cts.Token.IsCancellationRequested)
            {
                ...
                Output.WriteLine($"[{DateTime.UtcNow:HH:mm:ss}] Memory: {currentMemory / 1024 / 1024}MB (+{growthMB}MB), Total events: {totalEvents}");
            }
        });
    ...
    public async Task HTTP_HighRate_Sustained_1Hour()
    {
        ...
        tasks.Add(Task.Run(async () =>
        {
            while (DateTime.UtcNow - startTime < duration)
            {
                ...
                memorySnapshots.Add(currentMemory);
                var growthMB = (currentMemory - startMemory) / 1024 / 1024;
                Output.WriteLine($"[{DateTime.UtcNow - startTime:mm\\:ss}] Memory: {currentMemory / 1024 / 1024}MB (+{growthMB}MB), Requests: {requestCount}");
            }
        }));
    ...
    public async Task SignalR_Messages_Continuous_1Hour()
    {
        ...
        while (DateTime.UtcNow - startTime < duration)
        {
            ...
            if (messageCount % 300 == 0) // Every 5 minutes
            {
                ...
                memorySnapshots.Add(currentMemory);
                var growthMB = (currentMemory - startMemory) / 1024 / 1024;
                Output.WriteLine($"[{DateTime.UtcNow - startTime:mm\\:ss}] Memory: {currentMemory / 1024 / 1024}MB (+{growthMB}MB), Messages: {messageCount}");
            }
        ...
        var endMemory = GC.GetTotalMemory(true);
        var totalGrowthMB = (endMemory - startMemory) / 1024 / 1024;
        Output.WriteLine($"[END] Final memory: {endMemory / 1024 / 1024}MB, Growth: {totalGrowthMB}MB, Messages: {messageCount}");
    ...
    public async Task Tunnel_LongRunning_CreateDestroy_2Hours()
    {
        ...
        while (DateTime.UtcNow - startTime < duration)
        {
            ...
            if (tunnelCycles % 360 == 0) // Every 10 minutes
            {
                ...
                var growthMB = (currentMemory - startMemory) / 1024 / 1024;
                var elapsed = DateTime.UtcNow - startTime;
                Output.WriteLine($"[{elapsed:hh\\:mm}] Memory: {currentMemory / 1024 / 1024}MB (+{growthMB}MB), Tunnels: {tunnelCycles}");
            }
        ...
        var endMemory = GC.GetTotalMemory(true);
        var totalGrowthMB = (endMemory - startMemory) / 1024 / 1024;
        Output.WriteLine($"[END] Final memory: {endMemory / 1024 / 1024}MB, Growth: {totalGrowthMB}MB, Tunnels: {tunnelCycles}");
    ...
    public async Task Client_Reconnect_Pattern_1Hour()
    {
        ...
        while (DateTime.UtcNow - startTime < duration)
        {
            ...
            if (reconnectCycles % 10 == 0)
            {
                ...
                memorySnapshots.Add(currentMemory);
                var growthMB = (currentMemory - startMemory) / 1024 / 1024;
                Output.WriteLine($"[{DateTime.UtcNow - startTime:mm\\:ss}] Memory: {currentMemory / 1024 / 1024}MB (+{growthMB}MB), Reconnects: {reconnectCycles}");
            }
        ...
        var endMemory = GC.GetTotalMemory(true);
        var totalGrowthMB = (endMemory - startMemory) / 1024 / 1024;
        Output.WriteLine($"[END] Final memory: {endMemory / 1024 / 1024}MB, Growth: {totalGrowthMB}MB, Reconnects: {reconnectCycles}");
    ...
    public async Task MemoryPressure_UnderLoad_Continuous_4Hours()
    {
        ...
        Output.WriteLine($"[START] Initial memory: {startMemory / 1024 / 1024}MB");
        Output.WriteLine("This is a 4-hour soak test. Monitor memory externally with:");
        Output.WriteLine("  Windows: perfmon, dotMemory, PerfView");
        Output.WriteLine("  Linux: top, htop, dotnet-dump");
        ...
        var tasks = new[]
        {
            ...
            Task.Run(async () =>
            {
                while (!cts.Token.IsCancellationRequested)
                {
                    ...
                    Output.WriteLine($"[{elapsed:hh\\:mm\\:ss}] Memory: {currentMemory / 1024 / 1024}MB (+{growthMB}MB), " +
                    ...
                    if (growthMB > 400)
                    {
                        Output.WriteLine($"WARNING: Memory growth exceeds 400MB! Possible memory leak detected.");
                    }
                }
            })
        };
        ...
        Output.WriteLine($"Operations: HTTP: {stats.Http}, SSE: {stats.Sse}, SignalR: {stats.SignalR}, Errors: {stats.Errors}");
        ...
        foreach (var snapshot in memorySnapshots.TakeLast(10))
        {
            ...
            Output.WriteLine($"  [{elapsed:hh\\:mm}] {snapshot.Memory / 1024 / 1024}MB (GC: {snapshot.Gen0}/{snapshot.Gen1}/{snapshot.Gen2})");
        }
