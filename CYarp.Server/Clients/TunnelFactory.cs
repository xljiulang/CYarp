...
namespace CYarp.Server.Clients
{
    ...
    sealed class TunnelFactory
    {
        ...
        public async Task<Tunnel> CreateTunnelAsync(ClientConnection connection, TunnelType tunnelType, CancellationToken cancellationToken)
        {
            ...
            {
                throw new SystemException($"A tunnelId {tunnelId} already exists in the system");
            }
            ...
            {
                TunnelLog.LogTunnelCreateFailure(this.Logger, connection.ClientId, tunnelId, "Remote side operation timeout");
                throw;
            }
            catch (Exception ex)
            {
